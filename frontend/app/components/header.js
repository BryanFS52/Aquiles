...
export const Header = ({ role }) => {
  ...
  return (
    <header
      className={`w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[78vw] h-[9vh] min-h-[60px] rounded-xl mx-auto flex items-center justify-end px-3 sm:px-4 lg:px-5 py-2 lg:py-5 my-3 sm:my-4 lg:my-5 shadow-none ${bgColor}`}
    >
      {/* Notificaciones */}
      ...
      >
        <div className="hidden xs:flex flex-col text-end">
          ...
          <span className={`text-xs sm:text-sm lg:text-base ${roleColor}`}>
            {roleLabel}
          </span>
        </div>
        ...
        <div className="flex xs:hidden flex-col text-start">
          ...
          <span className={`text-xs ${roleColor} leading-tight`}>
            {roleLabel}
          </span>
        </div>
      </Link>
