...
export default function StudySheetsPage() {
  ...
  if (!studySheetLoading && (!fichas || fichas.length === 0)) {
    return (
    ...
    <div className="w-full px-2 sm:px-4 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        {fichas.map((studySheet: StudySheetWithCompetence, index: number) => (
          <Link
            key={studySheet.id}
            href={`/dashboard/InstructorFollowUp?ficha=${studySheet.number}`}
                      ...
                      <div className="space-y-1 sm:space-y-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs font-semibold text-green-700 dark:text-green-400">
                            {formatDate(studySheet.startLective ?? 'N/A')}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs font-semibold text-red-700 dark:text-red-400">
                            {formatDate(studySheet.endLective ?? 'N/A')}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs sm:text-sm font-semibold text-gray-600 dark:text-dark-textSecondary">
                      {studySheet.numberStudents} Aprendices
                    </span>
                  </div>
